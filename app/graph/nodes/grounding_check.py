...
def grounding_check_node(state: AgentState) -> AgentState:
    ...
    Returns:
        Updated state with grounding_result, confidence,
        and unsupported_claims populated.
    """

    # Skip if fallback was already triggered
...
def route_after_grounding(state: AgentState) -> str:
    """
    Conditional edge function after grounding check.

    Returns:
        "fallback_node" if confidence is LOW
        "stakes_assessor" if confidence is HIGH
    """

    if (
        state.get("confidence") == "LOW" or
        state.get("fallback_triggered") is True
    ):
